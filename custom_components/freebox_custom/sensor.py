...
from .const import DOMAIN, DeviceConnectivityType, WIFI_BANDS_TO_CONNECTIVITY
...
async def async_setup_entry(
    ...
    for device in router.devices.values():
        ...
        if get_device_connectity_type(device) != DeviceConnectivityType.ETHERNET:
            entities.extend(
                FreeboxDeviceSensor(router, device, description)
...
class FreeboxDeviceSensor(FreeboxSensor):
    ...
    def async_update_state(self) -> None:
        ...
        if self.entity_description.key == "connectivity":
            ...
            self._attr_native_value = get_device_wifi_signal_strength(self._device)
...
def get_device_wifi_signal_strength(device: dict[str, Any]) -> int | None:
    """Get the wifi signal strength of a device."""
    access_point = device.get("access_point", {})
    wifi_information = access_point.get("wifi_information", {})
    if dbm := wifi_information.get("signal"):
